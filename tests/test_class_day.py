...
from pylift.classes.day import Day, Workout
...
def test_str(rng: DayFactory) -> None:
    d = rng.random_date_str()
    day = Day(d[0], [])
    assert str(day) == d[1]


def test_repr(rng: DayFactory) -> None:
    date = rng.random_date()
    my_workouts = rng.random_workouts
    day = Day(date, my_workouts())
...
def test_to_dict() -> None:
    ...
    day = Day(dat, [workout])
    expected_dict = {"date": "2023-10-01", "workouts": [workout.to_dict()]}
    assert day.to_dict() == expected_dict


def test_save(rng: DayFactory, tmp_path: Path) -> None:
    d = rng.random_day()
    test_file = tmp_path / f"{d.date_str}.json"
    exercise_types = rng.exercise_types()

    d.save(d, exercise_types, tmp_path)
    d2 = Day.read(test_file)
    assert d == d2


def test_to_dataframe(rng: DayFactory) -> None:
    workout = Workout("Squat", [Set(10, 50.0), Set(12, 55.0)], "Good form")
    workout2 = Workout("Deadlift", [Set(8, 70.0), Set(10, 75.0)], "Felt strong")
