...
class DayFactory(WorkoutFactory):
    def random_day(self) -> Day:
        return Day(date=self.random_date(), workouts=self.random_workouts())
    ...
    def static_days(self) -> list[Day]:
        return [
            ...
            Day(
                date=date(2025, 4, 25),
                workouts=[Workout("Deadlift", [Set(10, 12), Set(10, 12)], note="")],
            ),
            Day(
                date=date(2025, 5, 18),
                workouts=[
                    Workout(
                        "Lateral Pulldown",
                        [Set(12, 25), Set(7, 31.5), Set(8, 31.5), Set(8, 31.5)],
                        note="",
                    ),
                ],
            ),
