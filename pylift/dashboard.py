...
def run_dashboard(database_path: Path, host: str, port: str, *, debug: bool) -> None:
    """
    ...
    def update_graph(selected_exercise: str) -> Figure:
        # Filter and aggregate
        filtered = df[df["Exercise"] == selected_exercise]
        summary = filtered.groupby("Date").agg(Max1RM=("Epley1RM", "max")).reset_index()
        fig = px.line(
            summary,
            x="Date",
            y="Max1RM",
